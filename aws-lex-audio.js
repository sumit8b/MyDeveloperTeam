...
(function () {
  ...
  exports.audioControl = function (options) {
    ...
    var startRecording = function (onSilence, visualizer, silenceDetectionConfig) {
      ...
      //alert('audioSupported',audioSupported)
      if (!audioSupported) {
...
(function() {
  ...
  window.AWS.config.credentials = new AWS.CognitoIdentityCredentials({
    ...
    this.advanceConversation = function() {
	  ...
	  //alert(state.lexconfig.inputStream)
      lexruntime.recognizeUtterance(state.lexConfig, function(err, data) {
		  //alert(data)
        if (err) {
			console.log(err);
          state.onError(err);
  ...
  var applyDefaults = function(config) {
    ...
    lexConfig.botAliasId = 'TSTALIASID';
    lexConfig.requestContentType = 'audio/x-l16; sample-rate=16000; channel-count=1';
    lexConfig.localeId = 'en_US';
    lexConfig.sessionId = '1234567890';
	lexConfig.responseContentType = 'text/plain;charset=utf-8';
